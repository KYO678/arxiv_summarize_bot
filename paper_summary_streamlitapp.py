...
def search_paper_by_title(title):
    ...
    try:
        ...
        search = arxiv.Search(
            query=query,
            max_results=3,  # 複数結果を取得して選択肢を増やす
        ...
        if "301" in error_msg:
            ...
            try:
                search = arxiv.Search(
                    query=title.split()[:3],  # 最初の3単語のみで検索
                    max_results=5
                )
                results = list(search.results())
                return results[0] if results else None
